...
14. [Seed Data Audit](#13-seed-data-audit)
...
### 13.1 Database Row Counts

The database seeding scripts populate the relational layout with a highly realistic Central Asian dataset. The following table details the audited row counts for every table in the schema:

| Table Name | Row Count | Description |
| :--- | :--- | :--- |
| `categories` | 8 | Hierarchical category classifications (ATC standards) |
| `medicines` | 210 | Catalog of distinct pharmaceutical products |
| `suppliers` | 55 | Local wholesale distributors and manufacturers |
| `patients` | 220 | Mapped patients with valid PINs and demographics |
| `doctors` | 15 | Authorized healthcare providers |
| `employees` | 6 | Mapped staff members (cashiers, pharmacists, managers) |
| `batches` | 630 | Inventory physical batches tracking stock and expiry |
| `prescriptions` | 320 | Medical script headers |
| `prescription_items` | 640 | Detailed items linked to prescriptions |
| `sales` | 520 | Checkout transaction headers |
| `sale_items` | ~1,040 | Individual transaction line items |
| `inventory_adjustments` | 12 | Manual stock audit events |

*Note: The `sale_items` table contains approximately 1,040 rows due to the programmatic generation mapping multiple items per sales checkout transaction.*

### 13.2 Data Sources

The seed dataset utilizes a hybrid approach combining curated, hand-written authoritative data with algorithmically generated structural profiles to simulate a live production environment:

- **Categories**: Hand-written following real-world Anatomical Therapeutic Chemical (ATC) classifications (e.g., Penicillins, Cephalosporins, NSAIDs) for absolute clinical consistency.
- **Medicines**: AI-generated using authentic International Nonproprietary Names (INNs) for generic components and realistic Central Asian equivalents for brand trade names.
- **Suppliers**: AI-generated programmatically via procedural loops. Company names are generated by combining Kyrgyz geographic nouns with standard commercial suffixes (e.g., "ОсОО Фарм Импорт") accompanied by valid 14-digit corporate TINs.
- **Doctors**: AI-generated using common Central Asian and Slavic personal names, combined with real clinic names across major Kyrgyz municipalities (e.g., "National Hospital of the Kyrgyz Republic").
- **Employees**: Hand-crafted containing 6 distinct, realistically named staff profiles to map functional role security and audit logs.
- **Patients**: AI-generated programmatically using gender-matched Kyrgyz and Slavic names. Each patient record contains a valid 14-digit Personal Identification Number (PIN) derived systematically from their date of birth, satisfying local regulatory standards.
- **Batches**: Algorithmic generation defining acquisition and retail prices within logical Kyrgyz Som (KGS) bands. Manufacturing and expiry dates are distributed over a 3-year window.
- **Prescriptions**: Algorithmic distribution tracking issuing events over a 6-month timeline, incorporating non-uniform status distribution.
- **Sales**: Algorithmic generation mapped to cashier IDs, incorporating realistic payment methods and checkout totals calculated dynamically by database triggers.
- **Inventory Adjustments**: Hand-crafted comprising 12 realistic audit events written in Russian and English (e.g., spoilage, retail breakage, count reconciliation) to verify physical adjustment ledger triggers.

### 13.3 Distribution Patterns Implemented

To test performance and trigger validity under stress, the following non-uniform statistical distributions were designed into the generation engine:

- **Time Clustering**: Sales transactions are concentrated exclusively within realistic business hours (08:00 to 20:00) over the preceding 6 months, avoiding uniform night-time anomalies.
- **Pareto-Like Employee Activity**: Transactions are distributed unevenly across cashiers. Employee IDs cycle within standard shifts using modulo logic, creating natural operational variation where some staff process significantly higher sales volumes.
- **Realistic Null Patterns**: Nullable fields, such as patient emails, are populated realistically with a ~30% null rate, reflecting incomplete client registration forms in real-world retail settings.
- **Realistic Status Distributions**: Prescriptions are generated with chronological decay: older scripts issued over 6 months ago are marked as expired (~60%), whereas active ones maintain a non-uniform distribution (~8% fully filled, ~5% partially filled, and the remainder pending).
- **Critical Edge Cases**:
  - Exactly 8% of all physical batches are generated with dates in the past to verify that safety triggers successfully block expired drug sales.
  - Exactly 5% of all active batches are initialized with zero inventory levels to validate out-of-stock database check constraints and low-stock dashboard alert views.

### 13.4 Distribution-Proof Verification Queries

The non-uniform operational distributions implemented in the seeding script can be verified directly using the following SQL queries:

#### Query 1: Employee Transaction Distribution
This query demonstrates the non-uniform distribution of sales checkout processing across the pharmacy staff:
...
SELECT 
    ...
    e.role,
    COUNT(s.id) AS total_processed_sales,
    ROUND((COUNT(s.id)::numeric / SUM(COUNT(s.id)) OVER ()) * 100, 2) AS share_percentage
FROM employees e
LEFT JOIN sales s ON e.id = s.employee_id
GROUP BY e.id, e.first_name, e.last_name, e.role
ORDER BY total_processed_sales DESC;
```

#### Query 2: Payment Method Diversity
This query verifies the implementation of diverse checkout payment channels to ensure transaction processing is not statistically flat:

```sql
SELECT 
    payment_method,
    COUNT(id) AS transaction_count,
    ROUND(SUM(total_net), 2) AS total_net_revenue_kgs,
    ROUND((COUNT(id)::numeric / SUM(COUNT(id)) OVER ()) * 100, 2) AS transaction_share_percent
FROM sales
...
This project was developed with AI assistance from Claude (claude-sonnet-4-6), an AI assistant by Anthropic.

**AI Contributions:**
- Helped design the database schema and ENUM type selection
- Wrote and debugged the PL/pgSQL trigger functions
- Generated the normalization proofs and BCNF analysis
- Generated the Kyrgyzstan-localized seed data DO blocks
- Wrote the RBAC role definitions and privilege grant statements
- Drafted the 15 analytical queries

**Human Contributions:**
- Defined the pharmacy business requirements and local Kyrgyz regulatory context (TIN, PIN format, phone format, 12% VAT rate)
- Selected the project domain and scoped the 12 core entities
- Reviewed and validated all AI-generated SQL against real PostgreSQL 15+ execution
- Verified seed data realism against local Kyrgyz pharmacy knowledge
- Corrected phone number formats, supplier names, and city distributions

**Prompts Used (Summary):**
All major design prompts were provided conversationally via the Claude Code CLI. Key prompt themes: schema design for pharmacy domain, normalization analysis, PL/pgSQL trigger logic, seed data generation in Kyrgyzstan context, and RBAC security policy.
